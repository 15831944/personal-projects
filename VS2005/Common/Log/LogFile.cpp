...
try{
	...
	if (_bEnableLogFile){
	 ...
	 #else
		sprintf_s(szPath,"%s%02d%02d%02d.log",_szLogFilePath,time.wYear,time.wMonth,time.wDay);			
		result=file.Open(szPath,CFile::modeCreate|CFile::modeNoTruncate|CFile::modeReadWrite);
	 #endif
		if (result){		
	...
	va_list args;
	va_start(args, szMsg);	
	nBuf = _vsntprintf_s(szBuffer, sizeof(szBuffer), szMsg, args);
	va_end(args);
...
#else
	sprintf_s(szLog,_T("[%02d:%02d:%02d]- %s\r\n"),time.wHour,time.wMinute,time.wSecond,szBuffer);	
	if (_bEnableLogFile){
		if (result){
