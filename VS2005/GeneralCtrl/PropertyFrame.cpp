...
int CPropertyFrame::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	m_pModelessPropSheet = new CPanelPropertySheet(this);

	m_pModelessPropSheet->AddPage(&m_pModelessPropSheet->m_PageGeneral);	
	...
	SetWindowPos(NULL, rectWindow.left, rectWindow.top,
		rectWindow.Width(), rectWindow.Height(),SWP_FRAMECHANGED);
	m_pModelessPropSheet->SetWindowPos(NULL, 0, 0,
		rectClient.Width(), rectClient.Height(),SWP_FRAMECHANGED);	
