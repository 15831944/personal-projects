...
#define MAXSEGCHAR7		13		// Number of supported 7 segment characters
...
void CDigiStatic::OnPaint() 
{
	...
	if (m_DispStyle & DS_SZ_PROP)
	{
		...
		CharRect.left = (rect.Width() - dRelWidth * iTotWidth) / 2 - m_CharArray.GetSize();
	...
	for (int iChar = 0; iChar < m_CharArray.GetSize(); iChar++)
	{
		...
		CharRect.left += dRelWidth * m_CharArray[iChar].GetNormWidth() +2;		
	}
...
void CDigiStatic::GetText(CString& csText) const
{
	csText = m_strText;
}
