...
{                                  
    ...
    for (i=0; i< 60; i++){      
        ...
        for (k=0; k< 8; k++){
            if ((data[0]&mask)>>(k)){
                PORTB.0 = 1;delay_us(1);
            }
            else{
                PORTB.0 = 0;delay_us(1);
            }    
                        if ((data[1]&mask)>>(k)){
                PORTB.1 = 1;delay_us(1);
            }
            else{
                PORTB.1 = 0;delay_us(1);
            }
                        if ((data[2]&mask)>>(k)){
                PORTB.2 = 1;delay_us(1);
            }
            else{
                PORTB.2 = 0;delay_us(1);
            }
                        if ((data[3]&mask)>>(k)){
                PORTB.3 = 1;delay_us(1);
            }
            else{
                PORTB.3 = 0;delay_us(1);
            }
                        if ((data[4]&mask)>>(k)){
                PORTB.4 = 1;delay_us(1);
            }
            else{
                PORTB.4 = 0;delay_us(1);
            }
...
static void TextFromFont(char *szText, unsigned char      nColor, unsigned char      bGradient, unsigned char     *    pBuffer, unsigned char      index)
{
	...
	for (i=0; i< len; i++){				                                     
		...
		nCurWidth = eeprom_read_w(devID,(unsigned char     *   )0x0A+(ch<<1));    
		nNxtWidth = eeprom_read_w(devID,(unsigned char     *   )0x0A+((ch+1)<<1));    
