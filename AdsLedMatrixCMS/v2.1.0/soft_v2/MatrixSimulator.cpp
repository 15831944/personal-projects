...
static BYTE __SCREEN_LAYER[MAX_LINE][LAYER_COUNT][DATA_LINE*DATA_LENGTH];
static BYTE __FONT_BUFFER[MAX_LINE][LAYER_COUNT][DATA_LINE*DATA_LENGTH];
/////////////////////////////////////////////////////////////////////////////
// CMatrixSimulator
...
CMatrixSimulator::CMatrixSimulator()
{
	...
	for (int i = 0; i< LAYER_COUNT; i++)
	{
		...
		for (int line =0; line < MAX_LINE; line++){
			memset(__SCREEN_LAYER[line][i],BLANK_STATE,DATA_LINE*DATA_LENGTH);	
			memset(__FONT_BUFFER[line][i],BLANK_STATE,DATA_LINE*DATA_LENGTH);
		}
	}
}
		...
		
		{
			...
			for (int l=LAYER_COUNT-1; l>= 0; l--)
			{
				if (m_bVisibleLayer[l])
					data = ExtORByte(data,__SCREEN_LAYER[m_nLine][l][bit_col + DATA_LENGTH*bit_row + (m_nCurrentPage+1)*m_cx]);
			}			
		}			
...
{
	int cx =0, nPixel =0;
	PBYTE pData = __SCREEN_LAYER[m_nLine][m_nCurrentLayer] + m_cx;
	if (m_bLoadImage[m_nCurrentLayer]==FALSE){
		memset(__SCREEN_LAYER[m_nLine][m_nCurrentLayer],BLANK_STATE,DATA_LINE*DATA_LENGTH);
	}
...
{
	*pBuffer = __SCREEN_LAYER[m_nLine][m_nCurrentLayer] + m_cx;
...
{
	PBYTE pBuffer = __SCREEN_LAYER[m_nLine][layer];
...
void CMatrixSimulator::MergeLayer(int nLayer)
{
	for (int y=0 ; y < DATA_LINE; y++)
	{
		...
		{
			m_pOrigin[x + DATA_LENGTH*y] = ExtORByte(m_pOrigin[x + DATA_LENGTH*y],__SCREEN_LAYER[m_nLine][nLayer][x + DATA_LENGTH*y]);			
		}
	}
...
void CMatrixSimulator::LoadImage(LPCTSTR szBmpFile, BOOL bBkGnd)
{
	...
	PBYTE pData = bBkGnd?m_pStaticLine:__SCREEN_LAYER[m_nLine][m_nCurrentLayer] + m_cx;

	if (!bBkGnd)
		memset(__SCREEN_LAYER[m_nLine][m_nCurrentLayer],0x00,DATA_LINE*DATA_LENGTH);
	else	
...
{
	PBYTE pBuffer = __SCREEN_LAYER[m_nLine][nLayer];
	for (int i =0; i< DATA_LENGTH*DATA_LINE; i++)
	{
		pBuffer[i] = ChangeColor(nColor,BYTE(pBuffer[i]));
	}	
...
{
	if (m_nTextLength > 255) m_nTextLength = 255;
...
{
	if (m_nFontTextLength > 255) m_nFontTextLength = 255;
...
void CMatrixSimulator::GradientLayer(int nColor, int nLayer)
{
	...
	if (nLayer>=0)
	{
		...
		pBuffer = (__SCREEN_LAYER[m_nLine][nLayer] + m_cx);
	}
	else
	{
		cx = m_cx;
...
void CMatrixSimulator::InitDefaultFont()
{
	for (int i =0; i < LAYER_COUNT; i++){
		for (int k =0; k< MAX_LINE; k++){
			LoadFont(__FONT_BUFFER[k][i],sizeof(__FONT_BUFFER[k][i]),i);	
		}
	}
}
...
{	
	LoadFont(__FONT_BUFFER[m_nLine][m_nCurrentLayer],sizeof(__FONT_BUFFER[m_nLine][m_nCurrentLayer]),m_nCurrentLayer);	
}
...
{
	*pBuffer = __SCREEN_LAYER[m_nLine][m_nCurrentLayer];
	
	memcpy(*pBuffer,__FONT_BUFFER[m_nLine][m_nCurrentLayer],DATA_LINE*DATA_LENGTH);
...
int CMatrixSimulator::TextFromFont(const char *szText, int nColor, BOOL bGradient, PBYTE *pBuffer, UINT nLenght)
{
	...
	{				
		...
		for (int y=0; y< 8 ; y++)
		{
			...
			{
				(*pBuffer)[x + pos + y*nLenght] = (~mask) & __FONT_BUFFER[m_nLine][m_nCurrentLayer][x + m_wFontCharWidth[m_nCurrentLayer][c] + y*DATA_LENGTH];								
			}					
		}
		pos += nWidth;	
	}	
...
void CMatrixSimulator::NextPage()
{
	m_nCurrentPage++;
	if ((m_nCurrentPage+1)*m_cx > DATA_LENGTH){
		m_nCurrentPage = 0;
	}
}

void CMatrixSimulator::PrevPage()
{
	m_nCurrentPage--;		
	if ((m_nCurrentPage+1)*m_cx < (UINT)m_cx){
		m_nCurrentPage = 0;
	}
}

UINT CMatrixSimulator::GetCurrentPage()
{
	return m_nCurrentPage;
}

void CMatrixSimulator::DoAdjust(BOOL bLeft)
{
	PBYTE pBuffer = __SCREEN_LAYER[m_nLine][m_nCurrentLayer];
	if (bLeft){
		for (int x=0; x< DATA_LENGTH-3; x++){
			for (int y=0; y< 8; y++){			
				BYTE data0 = 0;			
				BYTE data1 = pBuffer[x+1+y*DATA_LENGTH];
				data0 = pBuffer[x+0+y*DATA_LENGTH];
				pBuffer[x+y*DATA_LENGTH] = (data1&0x03) | (data0&0xFC);
				data0 = pBuffer[x+0+y*DATA_LENGTH];
				pBuffer[x+y*DATA_LENGTH] = (data1&0x30) | (data0&0xCF);
			}
		}
	}
	else {
		for (int x=DATA_LENGTH; x>=3; x--){
			for (int y=0; y< 8; y++){			
				BYTE data0 = 0;			
				BYTE data1 = pBuffer[x-1+y*DATA_LENGTH];
				data0 = pBuffer[x-0+y*DATA_LENGTH];
