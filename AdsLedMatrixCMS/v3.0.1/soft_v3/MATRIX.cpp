...
BOOL CMATRIXApp::InitInstance()
{
	...
	if (!GetPermission(__SETTING.m_csAppPath + _T("\\license.dat"),__SETTING.m_csLicense)){
		__SETTING.m_csLicense = _T("unregistered version!");
	}
...
BOOL CMATRIXApp::LoadSetting()
{
	...
	__SETTING.m_nColumn = GetProfileInt(pszKey,_T("m_nColumn"),167);
	...
	if (__SETTING.m_csAppPath==_T("")){
		GetCurrentDirectory(sizeof(szDir),szDir);
		WriteProfileString(pszKey,_T("m_csAppPath"),szDir);
	}
