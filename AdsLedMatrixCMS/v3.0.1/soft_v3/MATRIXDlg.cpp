...
BOOL CMATRIXDlg::OnInitDialog()
{
	...
	CString csWindowText = _T("Static LCMS\x0AE"L" - ");
...
void CMATRIXDlg::OnSelchangeListLayer() 
{
	...
	if (nLayer < 4){
		...
		m_ScrollMode[m_nLastLayer] = m_nScrollMode;
		m_nScrollMode = m_ScrollMode[nLayer];

		m_Rate[m_nLastLayer] = m_nRate;
		m_nRate = m_Rate[nLayer];
...
{	
	...
	TRY{
		...
		for (int i=0; i< 4; i++){
			file.Read(&m_Rate[i],sizeof(m_Rate[i]));	
			file.Read(&m_ScrollMode[i],sizeof(m_ScrollMode[i]));	
...
void CMATRIXDlg::OnFileSaveworkspace() 
{
	...
	TRY{
		...
		for (int i=0; i< 4; i++){
			file.Write(&m_Rate[i],sizeof(m_Rate[i]));	
			file.Write(&m_ScrollMode[i],sizeof(m_ScrollMode[i]));	
