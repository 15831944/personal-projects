...
void SaveToEEPROM(WORD address,WORD length,BYTE index)
{                             
    PBYTE temp = 0;      
    BYTE devID = EEPROM_DEVICE_BASE;
    devID += index<<1;      				
    ...
    length = address+4*(SCREEN_WIDTH+length);         
...
void LoadToRAM(WORD address,WORD length,BYTE index)
{                         
    PBYTE temp = 0;          
    BYTE devID = EEPROM_DEVICE_BASE;
    devID += index<<1;      				
...
{                 
    ...
    LoadToRAM(START_RAM_TEXT,text_length,index);
    stopping_count = 0;
    scroll_count = 0;
    is_stopping = 0;
    switch (scroll_type)
    ...
    case LEFT_RIGHT:
        ...
        break;                
    case RIGHT_LEFT:
        start_mem = START_RAM_TEXT + (text_length<<2); 
        break;
    case BOTTOM_TOP:                             
        scroll_count = SCREEN_HEIGHT<<1;
        start_mem = START_RAM_TEXT + (SCREEN_WIDTH<<2); 
        break;
    case TOP_BOTTOM:   
        scroll_count = 0;                     
        start_mem = START_RAM_TEXT + (SCREEN_WIDTH<<2); 
...
void SerialToRAM(WORD address,WORD length)                                             
{
    PBYTE temp = 0;          
    UINT i =0;     				
    temp   = address;    
...
void ProcessCommand()
{
    ...
    case LOAD_TEXT_MSG:
        {                
            ...
            BlankRAM(START_RAM,END_RAM);
            SerialToRAM(START_RAM_TEXT+4*(SCREEN_WIDTH),text_length);                
			start_mem = START_RAM_TEXT+4*(SCREEN_WIDTH);				
			SaveToEEPROM(START_RAM_TEXT,text_length,rx_lparam);
    ...
    case EEPROM_LOAD_ALL_MSG:
        {
            ...
            LoadToRAM(START_RAM_TEXT,text_length,rx_lparam); 
            start_mem = START_RAM_TEXT+4*(SCREEN_WIDTH);
        }
        break;  
    case POWER_CTRL_MSG:
