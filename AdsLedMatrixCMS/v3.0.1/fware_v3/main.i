  ...
  if (tick_count > scroll_rate){    
    ...
    case 0:                
        if (is_stopping==0){   
   	    ...
   	    if (start_mem == (0x500        + 192  *4)    + (192  <<2))
   	        is_stopping = 1;
   	    if (is_stopping ==1)
   	    {
       	...
       	if (start_mem > (0x500        + 192  *4)    +4*(text_length+192  +32))             
   	    {         
   	        frame_index++;         
                                                                            ...
                                                                            void LoadConfig(unsigned char      index)
{
    ...
    i2c_init();
    PORTB.4 = 1;
    scroll_rate = eeprom_read(devID, 0);    
    scroll_type = eeprom_read(devID, 1);    
    text_length = eeprom_read_w(devID, 2);    
    if (text_length > (3936 -192  )	){
        text_length= 0;            
    }
    if (scroll_type > 5){
        scroll_type = 5;
                       ...
                       void SaveTextLength(unsigned char      index)
{
    ...
    i2c_init();
    PORTB.4 = 1;   
    eeprom_write_w(devID, 2,text_length); 
    PORTB.4 = 0;   
}
...
{                     
    ...
    i2c_init();
    PORTB.4 = 1;  
    eeprom_write(devID, 0,rate);    
    eeprom_write(devID, 1,type);    
    PORTB.4 = 0;       
}
void SaveToEEPROM(unsigned int address,unsigned int length,unsigned char      index)
{                             
    ...
    {   
        ...
        eeprom_write_page( devID, temp, temp, 64   );	      
                      ...
                      void LoadToRAM(unsigned int address,unsigned int length,unsigned char      index)
{                         
    unsigned char     *    temp = 0;          
    unsigned char      devID = 0xa0;
    devID += index<<1;      				
...
{                 
    ...
    BlankRAM(0x500    ,0x8000);
    LoadToRAM((0x500        + 192  *4)   ,text_length,index);
    stopping_count = 0;
    scroll_count = 0;
    is_stopping = 0;
    switch (scroll_type)
    {
    case 0:
        start_mem = (0x500        + 192  *4)   ; 
        break;                
    case 1:
        start_mem = (0x500        + 192  *4)    + (text_length<<2); 
        break;
    case 3:                             
        scroll_count = 16            <<1;
        start_mem = (0x500        + 192  *4)    + (192  <<2); 
        break;
    case 2:   
        scroll_count = 0;                     
        start_mem = (0x500        + 192  *4)    + (192  <<2); 
...
void SerialToRAM(unsigned int address,unsigned int length)                                             
{
    unsigned char     *    temp = 0;          
    unsigned int i =0;     				
    temp   = address;    
...
void ProcessCommand()
{
    ...
    case 2 :
        {                
            ...
            BlankRAM(0x500    ,0x8000);
            SerialToRAM((0x500        + 192  *4)   +4*(192  ),text_length);                
			start_mem = (0x500        + 192  *4)   +4*(192  );				
			SaveToEEPROM((0x500        + 192  *4)   ,text_length,rx_lparam);
    ...
    case 10:
        {
            ...
            LoadToRAM((0x500        + 192  *4)   ,text_length,rx_lparam); 
            start_mem = (0x500        + 192  *4)   +4*(192  );
        }
        break;  
    case 16  :
