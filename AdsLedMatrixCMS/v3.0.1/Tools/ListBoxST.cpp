...
void CListBoxST::MeasureItem(LPMEASUREITEMSTRUCT lpMeasureItemStruct)
{
	...
	lpMeasureItemStruct->itemHeight += CY_BORDER;
...
void CListBoxST::DrawItem(LPDRAWITEMSTRUCT pDIStruct)
{
	...
	COLORREF		crNormal = RGB(100,100,100);//GetSysColor(COLOR_WINDOW);
	COLORREF		crSelected = GetSysColor(COLOR_HIGHLIGHT);
	COLORREF		crText = RGB(0,0,155);//GetSysColor(COLOR_WINDOWTEXT);
	...
	if (m_pImageList)
	{
		if (bIsSelected && (m_byRowSelect == ST_FULLROWSELECT) && !bIsDisabled)
			...
			crColor = bIsSelected?RGB(50,50,50):crNormal;
	...
	CBrush brRect(RGB(110, 110, 110));
	CPen* pOldPen = pDC->SelectObject(&pen);
	CBrush* pOldBr = pDC->SelectObject(&brRect);
	pDC->FrameRect(&rcItem, &brRect);
...
BOOL CListBoxST::OnReflectedDblclk() 
{
	...
	if (int(nIndex) < GetCount()){	
		...
		int cx = 20;
		m_pDataEdit->SetWindowPos(NULL,rect.left + m_szImage.cx + CX_BORDER,
			rect.top + (rect.Height()-20)/2,
			rect.Width()- (m_szImage.cx + CX_BORDER) -2,
			20,SWP_FRAMECHANGED);
		TCHAR szBuff[MAX_PATH];
		this->GetText(nIndex,szBuff);		
		this->m_pDataEdit->SetWindowText(szBuff);
		this->m_pDataEdit->EnableWindow(TRUE);
		this->m_pDataEdit->ShowWindow(SW_SHOW);		
		this->m_pDataEdit->SetSel(0,-1,FALSE);		
...
BOOL CListBoxST::PreTranslateMessage(MSG* pMsg) 
{
		...
		case VK_RETURN:
			if (this->m_nEditShow == SW_SHOW)
			{
				...
				if (csEdit!=_T("")){
					...
					this->InsertString(nIndex,csEdit,0);
				}
			}			
			break;
		}
...
void CListBoxST::InitDataEdit()
{
	...
	if (m_pDataEdit)
	{
		...
		GetWindowRect(&rect);
		ScreenToClient(&rect);
		rect.bottom = rect.top + m_szImage.cy + CY_BORDER;
		m_pDataEdit->Create(WS_CHILD|WS_BORDER|ES_AUTOHSCROLL,rect,this,2201);
...
void CListBoxST::OnLButtonDown(UINT nFlags, CPoint point) 
{
	this->m_nEditShow = SW_HIDE;
	this->m_pDataEdit->ShowWindow(SW_HIDE);
	this->m_pDataEdit->EnableWindow(FALSE);
