...
{
    WORD result = 0;
    result = eeprom_read(deviceID,address*2);
    result = (result<<8) | eeprom_read(deviceID,address*2+1);
    return result;
}
void eeprom_write_w(BYTE deviceID, WORD address, WORD data)
{
    eeprom_write(deviceID,address*2,data>>8);
    eeprom_write(deviceID,address*2+1,data&0x0FF);    
}
