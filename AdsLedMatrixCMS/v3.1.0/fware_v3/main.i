...
{                   
    ...
    {    temp[0] = (unsigned long)start_mem[((unsigned int)4)*(x) + 0];    temp[1] = (unsigned long)start_mem[((unsigned int)4)*(x) + 1];    temp[2] = (unsigned long)start_mem[((unsigned int)4)*(x) + 2];    temp[3] = (unsigned long)start_mem[((unsigned int)4)*(x) + 3];    dw_temp = ~((temp[0]&0x000000FF) | ((temp[1]<<8)&0x0000FF00) |               ((temp[2]<<16)&0x00FF0000) | ((temp[3]<<24)&0xFF000000));    if (is_half_top){        dw_temp = dw_temp>>scroll_updown;    }    else{        dw_temp = dw_temp<<scroll_updown;    }};                     
         buff2 = ~(unsigned char     )(dw_temp>>24)&0x000000FF;        
    for (y=0; y< 8; y++){                      					
    ...
    buff2 = ~(unsigned char     )(dw_temp>>16)&0x000000FF;
    for (y=0; y< 8; y++){                     					
        if (power_off) data_bit =1                          ; 
        data_bit = (buff2<<y) & 0x80;                            
        PORTB.2      = data_bit;  
        ...
        PORTB.0      = 0;	    		
             ...
                 buff1 = ~(unsigned char     )(dw_temp>>8)&0x000000FF;
    for (y=0; y< 8; y++){                      					
        if (power_off) data_bit =1                          ;                            
    ...
    buff1 = ~(unsigned char     )(dw_temp)&0x000000FF;
    for (y=0; y< 8; y++){                     					
        if (power_off) data_bit =1                          ; 
        data_bit = (buff1<<y) & 0x80;                            
        PORTB.2      = data_bit;                         
  ...
  if (tick_count > scroll_rate){    
    ...
    case 0:                
        if (is_stopping==0){   
       	...
       	if (start_mem > (0x500       )    +4*(text_length+544  +32))             
   	    {                  
...
{                             
    ...
    length = (unsigned int)address+4*(544  +length);         
    if (length%64   )
...
{                 
    ...
    case 2:  
        is_half_top =0;
        scroll_updown = 32            ;                           
        start_mem = (unsigned char     *   )(0x500       )    + (544  <<2); 
        break;
    case 3:   
        is_half_top =1; 
        scroll_updown = 32            ;                                           
...
void ProcessCommand()
{
    ...
    case 2 :
        {                
            ...
            BlankRAM((unsigned char     *   )0x500    ,(unsigned char     *   )0x8000);
            SerialToRAM((unsigned char     *   )(0x500       )   +4*(544  ),text_length);                
			start_mem = (unsigned char     *   )(0x500       )   +4*(544  );				
