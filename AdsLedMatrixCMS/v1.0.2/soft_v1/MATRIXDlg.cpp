	...
	: CDialog(CMATRIXDlg::IDD, pParent)
{
	...
	for (int i=0; i< 4; i++)
	{
		m_csTextLayer[i] = _T("");
		m_ColorText[i] = 0; 			
		m_GradientText[i] = FALSE;		
		m_csFontTextLayer[i].Format(__SETTING.m_csFontText,i+1);
	}
...
BOOL CMATRIXDlg::OnInitDialog()
{
	...
	if (m_Serial.Open(csCOM,m_hWnd,0)!=ERROR_SUCCESS)
	{
		...
		MessageBox(m_csCommStatus + _T(". Xin h\xE3"L"y ch\x1ECD"L"n c\x1ED5"L"ng kh\xE1"L"c v\xE0"L" kh\x1EDF"L"i \x111"L"\x1ED9"L"ng l\x1EA1"L"i ch\x1B0"L"\x1A1"L"ng tr\xEC"L"nh"),
	...
	{
		CString csLayer = _T("");
		csLayer.Format(_T("Layer %d"),i+1);
		m_ctrlLayerList.AddString(csLayer);		
	}	
	m_ctrlLayerList.AddString(_T("Bk-Gnd"));	
	...
	this->SetWindowText(_T("Tiny LCMS - Designed by CuongQuay\x099"L""));
	...
	pCombo->AddString(_T("Ch\x1EA1"L"y ch\x1EEF"L""));
	pCombo->AddString(_T("Bay ch\x1EEF"L""));	
	pCombo->AddString(_T("Nh\xE1"L"y n\x1EC1"L"n"));
	pCombo->AddString(_T("Hi\x1EC7"L"n Logo"));
	pCombo->AddString(_T("T\x1ED5"L"ng h\x1EE3"L"p"));
	pCombo->SetCurSel(0);
	m_nScrollMode = 0;
	this->UpdateData(FALSE);
...
void CMATRIXDlg::OnButtonClear() 
{
	...
	m_csTextLayer[nLayer] = m_csText = _T("");
	this->UpdateData(FALSE);
}
...
void CMATRIXDlg::OnButtonSet() 
{
	...
	SetConfigMsg(m_nRate, 1, m_nScrollMode, 0);
...
void CMATRIXDlg::OnDeviceDownloaddata() 
{
	...
	this->SendCommandMsg(LOAD_TEXT_MSG,nLength,nTextLength);
...
void CMATRIXDlg::OnSelchangeListLayer() 
{
	...
	if (nLayer < 4){
		...
		m_ColorText[m_nLastLayer] = m_nColorText; 
		m_ctrlMatrix.SetCurrentLayer(nLayer);
		
		m_csTextLayer[m_nLastLayer] = m_csText;
		m_csText   = m_csTextLayer[nLayer];

		m_GradientText[m_nLastLayer] = m_bGradientText;
		m_bGradientText   = m_GradientText[nLayer];

		m_csFontTextLayer[m_nLastLayer] = m_csFontText;
		m_csFontText   = m_csFontTextLayer[nLayer];	

		m_ctrlMatrix.LoadCharMap(strFolderName + _T("\\") + m_csFontText);

		m_nLastLayer = nLayer;
		m_nColorText = m_ColorText[nLayer];			
...
void CMATRIXDlg::OnCloseupComboColorText() 
{
	...
	m_ctrlMatrix.Invalidate(FALSE);	
	m_EditText.SetTextColor(m_nColorText+1);
	m_ColorText[nLayer] = m_nColorText;
	this->UpdateData(FALSE);
}
...
{	
	...
	for (int row = 0; row < 8; row++)
	{
		...
		if ((nMsgType==LOAD_TEXT_MSG) || (nMsgType==LOAD_FONT_MSG))
...
{
	this->SetConfigMsg(m_nRate,0,m_nScrollMode,1);
}

void CMATRIXDlg::OnDevicePoweroncommand() 
{
	this->SetConfigMsg(m_nRate,1,m_nScrollMode,0);
}
...
void CMATRIXDlg::OnCheckGradientText() 
{
	this->UpdateData(TRUE);
	int nLayer = m_ctrlLayerList.GetCurSel();
	m_GradientText[nLayer] = m_bGradientText;
	if (m_bGradientText)
	{
		m_ctrlMatrix.GradientLayer(m_ColorText[nLayer]+1,nLayer);
		m_ctrlMatrix.Invalidate(FALSE);
	}
	else
	{
...
void CMATRIXDlg::OnDeviceDownloadbuildincharacterfont() 
{
	...
	this->SendData(LOAD_FONT_MSG,buffer,nLength);	
	this->SendExtra(char_width,int(nTextLength*sizeof(WORD)));
...
{	
	...
	TRY{
		...
		for (int i=0; i< 4; i++){
			file.Read(&m_ColorText[i],sizeof(m_ColorText[i]));	
			file.Read(&m_GradientText[i],sizeof(m_GradientText[i]));	
			TCHAR szText[255];
			file.Read(szText,255*sizeof(TCHAR));	
			m_csTextLayer[i] = szText;
			file.Read(szText,255*sizeof(TCHAR));	
			m_csFontTextLayer[i] = szText;
		}
...
void CMATRIXDlg::OnFileSaveworkspace() 
{
	...
	TRY{
		...
		for (int i=0; i< 4; i++){
			file.Write(&m_ColorText[i],sizeof(m_ColorText[i]));	
			file.Write(&m_GradientText[i],sizeof(m_GradientText[i]));	
			file.Write(m_csTextLayer[i].GetBuffer(m_csTextLayer[i].GetLength()),255*sizeof(TCHAR));	
			file.Write(m_csFontTextLayer[i].GetBuffer(m_csFontTextLayer[i].GetLength()),255*sizeof(TCHAR));	
		}
