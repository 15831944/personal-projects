...
CCustomTabCtrl::~CCustomTabCtrl()
{
	for(int i=0; i< (int)m_aItems.GetSize(); i++)
		delete m_aItems[i];
	m_aItems.RemoveAll();
...
BOOL CCustomTabCtrl::NotifyParent(UINT code, int nItem, CPoint pt)
{
	...
	if(nItem>=0)
	{
		_tcscpy_s(nmh.pszText,MAX_LABEL_TEXT,m_aItems[nItem]->m_sText);
...
void CCustomTabCtrl::OnMouseMove(UINT nFlags, CPoint point)
{
	...
	if(m_nButtonIDDown>=0 && m_nItemDragDest>=0)
	{
		...
		if(m_nItemDragDest>=(int)(int)m_aItems.GetSize())
			m_nItemDragDest = (int)(int)m_aItems.GetSize()-1;
		...
		if(x>=rCl.right)
		{
			m_nItemDragDest++;
			if(m_nItemDragDest>=(int)(int)m_aItems.GetSize())
				RecalcLayout(RECALC_NEXT_PRESSED,(int)(int)m_aItems.GetSize()-1);
			else
		...
		else if(x>=x2)
		{
			m_nItemDragDest++;
			if(m_nItemDragDest>=(int)(int)m_aItems.GetSize())
				RecalcLayout(RECALC_ITEM_SELECTED,(int)(int)m_aItems.GetSize()-1);
			else
...
{
	if(nItem<0 || nItem>(int)(int)m_aItems.GetSize())
	...
	{
		for(int i=(int)(int)m_aItems.GetSize()-1; i>nItem; i--)
		{
			CString s;
...
{
	if(nItemSrc<0||nItemSrc>=(int)m_aItems.GetSize())
		return CTCERR_INDEXOUTOFRANGE;
	if(nItemDst<0||nItemDst>(int)m_aItems.GetSize())
		return CTCERR_INDEXOUTOFRANGE;
	...
	if(m_ctrlToolTip.m_hWnd)
	{
		...
		for(int i=nItemSrc+1; i< (int)m_aItems.GetSize(); i++)
		{
			CString s;
...
int CCustomTabCtrl::CopyItem(int nItemSrc, int nItemDst, BOOL fMouseSel)
{
	...
	try
	{
		...
		while(1)
		{
			...
			int i=0;
			for(i=0;i<(int)m_aItems.GetSize();i++)
			{
				if(m_aItems[i]->m_sText==sDst)
					break;
			}
			if(i==(int)m_aItems.GetSize())
				break;
		}
	}
	catch(CMemoryException* e)
...
{
	if(nItem<0 || nItem>=(int)m_aItems.GetSize())
		return CTCERR_INDEXOUTOFRANGE;
	...
	try
	{
		if(m_ctrlToolTip.m_hWnd)
		{
			for(int i=nItem; i<(int)m_aItems.GetSize(); i++)
			{
				m_ctrlToolTip.DelTool(this,i+1);
				if(i!=(int)m_aItems.GetSize()-1)
				{
					CString s;
	...
	if((int)m_aItems.GetSize()==1)
		m_nItemSelected = -1;
	else if(m_nItemSelected==nItem)
	{
		if(m_nItemSelected==(int)m_aItems.GetSize()-1) // last item
		{
			m_nItemSelected--;
...
int CCustomTabCtrl::SetItemTooltipText(int nItem, CString sText)
{
	...
	if(nItem>=CTCID_CLOSEBUTTON && nItem<(int)m_aItems.GetSize())
	{
...
void CCustomTabCtrl::RecalcLayout(int nRecalcType, int nItem)
{
	...
	if((GetStyle()&CTCS_AUTOHIDEBUTTONS) && ((int)m_aItems.GetSize()<2 || nWidth <= rCl.Width()-nBnWidth))
	{
		m_nFirstState = BNST_INVISIBLE;
		m_nPrevState = BNST_INVISIBLE;
		m_nNextState = BNST_INVISIBLE;
	...
	case RECALC_NEXT_PRESSED:
		{	
			RecalcOffset(nBnWidth);
			if(m_aItems[(int)m_aItems.GetSize()-1]->m_rect.right>rCl.Width() && m_nItemNdxOffset!=(int)m_aItems.GetSize()-1)
			{
				m_nItemNdxOffset++;
				RecalcRectangles();
				RecalcOffset(nBnWidth);
	...
	default:	// window resized
		{
			...
			RecalcOffset(nBnWidth);
			while(m_nItemNdxOffset>=0 && m_aItems[(int)m_aItems.GetSize()-1]->m_rect.right<rCl.Width())
			{
				m_nItemNdxOffset--;
				if(m_nItemNdxOffset>=0)
				{
...
void CCustomTabCtrl::RecalcOffset(int nOffset)
{
	...
	for(int i = 0; i<(int)m_aItems.GetSize(); i++)
	{
		...
		else
		{
			...
			else if(i==(int)m_aItems.GetSize()-1)	// last item
				m_aItems[i]->m_bShape = TAB_SHAPE4;
			else
